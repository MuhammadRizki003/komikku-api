...
function getRecommendedManga($baseURL)
{
    $output = new Result();
    $result = array();
    ...
    foreach ($bgeDivs as $bge) {
        ...
        foreach ($bgeiDivs as $bgei) {
            $comic->Image = rtrim($bgei->find('img', 0)->getAttribute('data-src'), '?resize=450,235&quality=60');
            $comic->Endpoint = '/' . str_replace('https://komiku.id/', '', $bgei->find('a', 0)->getAttribute('href'));
            $comic->Type = $bgei->find('b', 0)->plaintext;
        }
